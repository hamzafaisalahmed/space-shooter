...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[node distance=1.4cm and 1.2cm]
        ...
        % ── BulletType tree ───────────────────────────────────────────────
        \node[abstract] (BT)  {\textit{BulletType}\\(abstract)};
        \node[block, below left=1.2cm and -0.2cm of BT]  (PN)  {PlayerNorm\\Bullet};
        \node[block, below=1.2cm of BT]   (EN)  {EnemyNorm\\Bullet};
        \node[block, below right=1.2cm and 0.6cm of BT]  (EB)  {EnemyBurst\\Bullet};
        \node[block, right=0.5cm of EB]   (BB)  {BossBeam\\Bullet};
        \draw[line] (PN) -- (BT);
        \draw[line] (EN) -- (BT);
        \draw[line] (EB) -- (BT);
        \draw[line] (BB.north) -- ++(0,0.4) -| (BT.south);

        % ── EnemyType tree ────────────────────────────────────────────────
        \node[abstract, right=4.5cm of BT] (ET) {\textit{EnemyType}\\(abstract)};
        \node[block, below left=1.2cm and 0.0cm of ET]  (SE) {SmallEnemy\\Type};
        \node[block, below=1.2cm of ET]    (ME) {MediumEnemy\\Type};
        \node[block, below right=1.2cm and 0.0cm of ET] (BE) {BossEnemy\\Type};
        \draw[line] (SE) -- (ET);
        \draw[line] (ME) -- (ET);
        \draw[line] (BE) -- (ET);

        % ── Level tree ────────────────────────────────────────────────────
        \node[abstract, right=3.8cm of ET] (LV) {\textit{Level}\\(abstract)};
        \node[block, below left=1.2cm and 0.0cm of LV]  (L1) {LevelOne};
        \node[block, below=1.2cm of LV]    (L2) {LevelTwo};
        \node[block, below right=1.2cm and 0.0cm of LV] (L3) {LevelThree};
        \draw[line] (L1) -- (LV);
        \draw[line] (L2) -- (LV);
        \draw[line] (L3) -- (LV);

        % ── Legend ────────────────────────────────────────────────────────
        \node[abstract, right=2.5cm of L3, yshift=1.2cm] (lgA) {\textit{Abstract}};
        \node[block, below=0.5cm of lgA] (lgC) {Concrete};
        \node[right=0.1cm of lgA, font=\small] {= has pure virtual};
        \node[right=0.1cm of lgC, font=\small] {= fully implemented};

    \end{tikzpicture}
    \caption{Three inheritance hierarchies. Arrows point from child to parent
             (\emph{``inherits from''}). Italic boxes are abstract (cannot be
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[node distance=1.2cm and 1.8cm]
        \node[abstract, fill=primary!15, draw=primary, text width=2cm] (G) {\textbf{Game}};

        \node[block, above left=1.2cm and 2.8cm of G]  (P)  {Player};
        \node[block, above=1.6cm of G]                 (GS) {GameState\\(stack)};
        \node[block, above right=1.2cm and 2.8cm of G] (LV) {Level*\\(vector)};

        \node[struct, below left=1.2cm and 3.2cm of G]  (OB) {ObjectPool\\$<$Bullet, 700$>$};
        \node[struct, below left=1.2cm and 0.8cm of G]  (OE) {ObjectPool\\$<$Enemy, 80$>$};
        \node[struct, below right=1.2cm and 0.8cm of G] (OP) {ObjectPool\\$<$Particle, 1000$>$};
        \node[struct, below right=1.2cm and 3.2cm of G] (ST) {Star\\array[200]};

        \draw[dashed_line] (G) -- (P)   node[midway, above, font=\tiny]{has-a};
        \draw[dashed_line] (G) -- (GS)  node[midway, right, font=\tiny]{has-a};
        \draw[dashed_line] (G) -- (LV)  node[midway, above, font=\tiny]{has-a};
        \draw[dashed_line] (G) -- (OB)  node[midway, left, font=\tiny]{has-a};
        \draw[dashed_line] (G) -- (OE)  node[midway, left, font=\tiny]{has-a};
        \draw[dashed_line] (G) -- (OP)  node[midway, right, font=\tiny]{has-a};
        \draw[dashed_line] (G) -- (ST)  node[midway, right, font=\tiny]{has-a};
    \end{tikzpicture}
    \caption{Composition: the \texttt{Game} class \emph{owns} (has-a) all subsystems.
             Dashed arrows mean ``contains as a member''.
             Orange boxes are template instances; blue boxes are plain classes.}
    \label{fig:compose}
\end{figure}
...
\begin{lstlisting}[caption={Player class --- private data, public interface (Player.h)}]
class Player
{
private:
    ...
    int   score;
    float shootTimer;       // time until next shot is allowed
    float iframeTimer;      // invincibility frames remaining after a hit
    float shieldTimer;      // if > 0, absorbs the next hit
    float tilt;             // visual lean when moving left/right
    bool  alive;
...
public:
    ...
    // Controlled mutation (setters + behaviour methods)
    void move(sf::Vector2f delta)  { pos += delta; }
    void clampToArena();           // keeps pos inside the play area
...
\textbf{Why this matters:} If \texttt{hp} were \texttt{public}, any part
of the code could write \texttt{player.hp = -999} by accident. The
\texttt{private} keyword makes that a compile error. Only the controlled
methods \texttt{takeDamage()} and \texttt{heal()} can change health, and
they do so safely.
...
\begin{lstlisting}[caption={BulletType inheritance hierarchy (BulletType.h)}]
...
public:
    ...
    virtual ~BulletType() {}          // virtual destructor -- essential!
    int  getId()           const { return id; }
    bool isPlayerBullet()  const { return playerBullet; }

    // Global singleton pointers -- one per bullet type, shared by all bullets
...
// Four concrete subclasses -- each calls the base constructor with its own id
...
\begin{lstlisting}[caption={EnemyType hierarchy (EnemyType.h)}]
class EnemyType {
    float maxHp;
    float shootInterval;
    int   scoreValue;
...
public:
    EnemyType(float maxHp, float shootInterval, int scoreValue,
        ...
        : maxHp(maxHp), shootInterval(shootInterval),
          scoreValue(scoreValue), moveSpeed(moveSpeed),
...
// Stats baked into each subclass constructor
...
\begin{lstlisting}[caption={Level abstract base class (Level.h)}]
class Level {
protected:
    std::string            name;
    std::string            desc;
    std::vector<SpawnEvent> events;   // the timed wave schedule
...
public:
    ...
    // Pure virtual -- every Level MUST implement these
    virtual float       getDuration()           const = 0;
    virtual std::string getPlanetTexturePath()  const = 0;
    virtual void        buildWaves()                  = 0;

    // Virtual with a default -- subclasses can override if needed
    virtual void renderPlanet(sf::RenderWindow& window);
    void loadPlanet();
};

// Each concrete level sets its own name, duration, and wave schedule
...
public:
    ...
    void        buildWaves()                 override;   // defined in Level.cpp
};
\end{lstlisting}
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[node distance=1.5cm and 2.5cm]
        \node[block] (home)  {Home};
        \node[block, right=of home]   (sel)   {Level\\Select};
        \node[block, right=of sel]    (play)  {Playing};
        \node[block, above=of play]   (pause) {Paused};
        \node[block, below right=1cm and 0.8cm of play] (over) {Game\\Over};
        \node[block, above right=1cm and 0.8cm of play] (done) {Level\\Complete};

        \draw[line] (home)  -- (sel)   node[midway, above, font=\tiny]{PLAY};
        \draw[line] (sel)   -- (play)  node[midway, above, font=\tiny]{PLAY};
        \draw[line] (play)  -- (pause) node[midway, right, font=\tiny]{P / Esc};
        \draw[line] (pause) -- (play)  node[midway, left, font=\tiny]{P / click};
        \draw[line] (play)  -- (over)  node[midway, right, font=\tiny]{lives = 0};
        \draw[line] (play)  -- (done)  node[midway, right, font=\tiny]{all enemies dead};
        \draw[line] (over)  |- (play)  node[midway, below, font=\tiny]{click};
        \draw[line] (done)  -| (sel)   node[near end, above, font=\tiny]{click};
        \draw[line] (pause)  -| (sel)  node[near end, above, font=\tiny]{EXIT TO MENU};
    \end{tikzpicture}
...
\begin{table}[H]
    ...
    \begin{tabular}{|c|p{3.5cm}|p{3.5cm}|p{2.5cm}|c|}
        \hline
        \textbf{TC} & \textbf{Action} & \textbf{Expected} & \textbf{Actual} & \textbf{Pass?} \\
        \hline
        TC-01 & Launch game & Home screen appears with stars and planets & As expected & \checkmark \\
        \hline
        TC-02 & Click PLAY & Level Select screen shown with 3 planets & As expected & \checkmark \\
        \hline
        TC-03 & Click a planet and press PLAY & Level starts; enemies appear on schedule & As expected & \checkmark \\
        \hline
        TC-04 & Press P during gameplay & Pause overlay appears; game freezes & As expected & \checkmark \\
        \hline
        TC-05 & Take damage until HP = 0 (with 2 lives left) & Life lost; HP refills; iframes granted & As expected & \checkmark \\
        \hline
        TC-06 & Lose all 3 lives & Game Over overlay shown with final score & As expected & \checkmark \\
        \hline
        TC-07 & Kill all enemies including boss & Level Complete overlay shown & As expected & \checkmark \\
        \hline
        TC-08 & Fire bullets rapidly (hold Space) & Up to 700 bullets active; no crash & As expected & \checkmark \\
        \hline
        TC-09 & Let 1000 particles appear at once & No slowdown; pool reuses slots correctly & As expected & \checkmark \\
        \hline
        TC-10 & Delete \texttt{Level*} via base pointer & Correct destructor called; no memory leak & As expected & \checkmark \\
        \hline
    \end{tabular}
\end{table}
